...
def run_model(trial, gain_in, threshold_in, seed=0):
...
def run_trials(n_trials=48, gain=0, threshold=0.5, plot=True, seed=0):
    corrects_simulated = np.zeros((n_trials, 1))
    ncues_simulated = np.zeros((n_trials, 1))
    for trial in range(n_trials):
        print('\ntrial: ', trial)
        data = run_model(trial, gain, threshold, seed)
