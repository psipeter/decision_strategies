...
def run_model(trial, gain, default):

    values_A, values_B = read_values(trial)
    values = [[values_A[n], values_B[n]] for n in range(len(values_A))]

    gain_process = nengo.processes.PresentInput([[gain]], presentation_time=6.0)
    default_process = nengo.processes.PresentInput([[default]], presentation_time=6.0)
    ...
    with model:
        ...
        gain_inpt = nengo.Node(gain_process)
        default_inpt = nengo.Node(default_process)
        ...
        default = nengo.Ensemble(1000, 1, radius=3)
...
def run_trials(
    ...
    for trial in range(n_trials_train):
        ...
        if correct and ncues_model > ncues_target:  # correct and late
            gain += delta_gain * (1-strategy)
            default -= delta_default * (1-strategy)
        if not correct and ncues_model < ncues_target:  # incorrect and early
            gain -= delta_gain * strategy
            default += delta_default * strategy
...
run_trials(participant=0, plot=True, n_trials_train=3, n_trials_test=4, strategy=0)
